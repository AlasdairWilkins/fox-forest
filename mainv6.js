function Game(choice, playera, playerb) {
    ...
    this.scoreTrick = function () {
        console.log("Let's score this thing!")
    ...
    this.start2p = function (datatemp) {
        ...
        this.buildWhoLeads(elem, datatemp['turn'])
        this.start2pTurn(elem, datatemp['turn'])
    ...
    this.start2pTurn = function(elemtemp, leadplayer) {
        console.log("Made it to the next round!")
        console.log(elemtemp)
        console.log(leadplayer)

        document.getElementById(elemtemp).addEventListener("animationend", function () {
            document.getElementById(elemtemp).style.display = "none";
...
function Player(name, id) {
    ...
    this.waitForTurn = function (suittemp) {
        console.log("Wait time!")
        var turnwait = setInterval(function () {
            console.log("Next step!")
            $.post("http://localhost:8000/turnwaiting", {'player': game.displayplayer.id}, function (data, status) {
                console.log("Okay and here we go!")
                if (!data['resend']) {
                    ...
                    if (game.trick.length === 1) {
                        ...
                        game.displayplayer.buildTrick()
                        game.displayplayer.completeRound(suittemp)
                    }
                } else {
    ...
    this.clicked = function (oldcount, x, y) {
        ...
        if (game.ai) {
            ...
            state = {'decree': game.decree, 'trick': game.trick, 'turn': this.id, 'completed': true};
            if (this === game.leadplayer) {
                console.log("Ahoy hoy hoy")
                $.post("http://localhost:8000/turncompleted", state, this.waitForTurn(x))
            } else {
                $.post("http://localhost:8000/turncompleted", state, this.completeRound(x))
                }
        }
    }
        ...
        if (player1.tricks.length + player2.tricks.length === 13) {
            ...
            document.getElementById(elem).addEventListener("animationend", function () {
                ...
                if (game.ai) {
                    ...
                    game.start2pTurn(elem, game.displayplayer)
                }
