function Game() {
    ...
    this.buildDecree = function() {
        document.getElementById("Decree").innerHTML = `The decree card is the ${this.decree.value} of ${this.decree.suit}.`
    };
    ...
    this.newRound = function() {
        ...
        this.shuffleDeck();
        this.resetPlayers();
        this.followplayer.buildHand(this.deck);
        this.leadplayer.buildHand(this.deck);
        game.setDecree();
    };
...
//    this.followMonarch = function {
//
//    }

//    this.resetMonarch = function {
//
//    }
...
function Player(name) {
    ...
    this.buildWoodcutterList = function (x, y) {
        for (var i = 0; i < this.hand.length; i++) {
            var newarray = this.hand[i].array.map(function (element) {
                var z = game.displayplayer.hand[i].array.indexOf(element);
                if (element.playable) {
                    return `<button class=${element.suit} id=${element.value} onclick='game.displayplayer.doWoodcutter(${i}, ${z}, ${x}, ${y})'>${element.value} of ${element.suit}</button>`
                } else {
    ...
    this.buildFoxList = function (x, y) {
        for (var i = 0; i < this.hand.length; i++) {
            var newarray = this.hand[i].array.map(function (element) {
                var z = game.displayplayer.hand[i].array.indexOf(element);
                return `<button class=${element.suit} id=${element.value} onclick='game.displayplayer.doFox(${i}, ${z}, ${x}, ${y})'>${element.value} of ${element.suit}</button>`
            }).join("")
            document.querySelector(`#${this.hand[i].suit}`).innerHTML = newarray
        }
    }
    ...
    this.doFox = function(suit, value, backupsuit, backupvalue) {
    ...
    this.doWoodcutter = function(suit, value, backupsuit, backupvalue) {
        var discard = this.hand[suit].array[value]
        this.hand[suit].array.splice(value, 1)
        game.deck.splice(0, 0, discard)
        if (this === game.displayplayer) {
            this.clicked(backupsuit, backupvalue)
        }
    }
    ...
    this.doWoodcutterAI = function() {
        ...
        console.log(game.deck[0].suit, game.deck[0].value)
        console.log(game.deck[game.deck.length-1].suit, game.deck[game.deck.length-1].value)
